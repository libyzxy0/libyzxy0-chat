...
<template>
  ...
  <div class="container">
    <ChatCard
      ...
      :image="`https://api.nilskoepke.com/profile-image?name=${item.firstName}+${item.lastName}&backgroundColor=rgb(${Math.floor(Math.random() * 256)},${Math.floor(Math.random() * 256)},${Math.floor(Math.random() * 256)})`"
    />
  </div>
</template>
<style scoped>
